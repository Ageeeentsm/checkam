...
async function googleResolve(query) {
  ...
  try {
    // Search CAC portal + Nigerian business sources
    const q = `"${query}" Nigeria company CAC RC number registered`;
    const url = `https://www.googleapis.com/customsearch/v1?key=${GOOGLE_KEY}&cx=${GOOGLE_CX}&q=${encodeURIComponent(q)}&num=5`;
    const {s,b} = await get(url);
    if(s !== 200) return [];
    const j = JSON.parse(b);
    return (j.items||[]).map(item => {
      // Try to extract RC number from snippet
