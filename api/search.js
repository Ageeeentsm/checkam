...
// ══════════════════════════════════════════════════════════════════
// CAC INTELLIGENCE ENGINE — Nigerian Corporate Affairs Commission
// Strategies: RC lookup → name search → fuzzy variants → HTML scrape → SerpApi
// Extracts: registration, directors, shareholders, related companies, history
// ══════════════════════════════════════════════════════════════════
async function scanCAC(entity, isCompany) {
  const out = { source:"CAC Nigeria", found:false, data:{}, raw_records:[], confidence:"low" };
  const trimmed = entity.trim();

  // ── Detect RC number input (RC123456 or just 123456) ──────────
  const isRC = /^(RC\s*)?(\d{4,8})$/i.test(trimmed);
  const rcNum = isRC ? trimmed.replace(/^RC\s*/i,"") : "";

  // ── Build smart name variants for fuzzy search ─────────────────
  ...
  const nameVariants = [...new Set([
    trimmed, clean, noSuffix,
    noSuffix + " LIMITED",
    noSuffix + " LTD",
    noSuffix + " PLC",
    noSuffix + " NIGERIA LIMITED",
    firstTwo, firstWord,
  ])].filter(v => v.length >= 2);

  // ── CAC API endpoints to try ───────────────────────────────────
  const CAC_ENDPOINTS = [
    ...
    "https://search.cac.gov.ng/home/search?name=",
  ];

  let records = [];

  if (isCompany) {
    // ── Strategy 1: Direct RC number lookup ─────────────────────
    if (isRC) {
      const rcEndpoints = [
        ...
        `https://search.cac.gov.ng/home/searchSimilarBusiness?name=RC${rcNum}`,
      ];
      for (const ep of rcEndpoints) {
        const {s,b} = await get(ep);
        if (s===200 && b) { records = parseCACRecords(b); if (records.length) break; }
      }
      if (records.length) out.confidence = "high";
    }

    // ── Strategy 2: Name search with variants ───────────────────
    if (!records.length) {
      outer: for (const ep of CAC_ENDPOINTS) {
        for (const v of nameVariants) {
          const {s,b} = await get(ep + encodeURIComponent(v));
          if (s===200 && b) {
            const found = parseCACRecords(b);
            if (found.length) {
              // Score matches — prefer exact name matches
              const scored = found.map(r => {
                const rName = (extractField(r,"company_name","CompanyName","name")||"").toLowerCase();
                const qName = v.toLowerCase();
                const score = rName===qName ? 3 : rName.includes(qName) ? 2 : qName.includes(rName) ? 1 : 0;
                return {...r, _score:score};
              }).sort((a,b)=>b._score-a._score);
              records = scored;
              out.confidence = scored[0]?._score>=2 ? "high" : scored[0]?._score>=1 ? "medium" : "low";
              break outer;
            }
          }
        }
    ...
    // ── Strategy 3: HTML scrape of CAC public search ────────────
    if (!records.length) {
      const urls = [
        `https://search.cac.gov.ng/home?name=${encodeURIComponent(clean)}`,
        `https://search.cac.gov.ng/home?name=${encodeURIComponent(noSuffix)}`,
      ];
      for (const url of urls) {
        const {s,b} = await get(url);
        if (s===200 && b) {
          // Extract RC numbers and company names from HTML
          const rcMatches  = b.match(/RC\s*\d{4,8}/gi) || [];
          const namePattern = />([A-Z][A-Z\s&().,'\-]{4,80}(?:LIMITED|LTD|PLC|NIGERIA))</gi;
          const nameMatches = [];
          let m;
          while ((m = namePattern.exec(b)) !== null) nameMatches.push(m[1].trim());
          if (rcMatches.length) {
            records = rcMatches.slice(0,5).map((rc,i) => ({
              company_name: nameMatches[i] || entity,
              rc_number: rc.replace(/\s/g,""),
              status: "Active",
              _source: "html_scrape"
            }));
    ...
    // ── Strategy 4: SerpApi intelligence extraction ─────────────
    if (!records.length && SERPAPI_KEY) {
      const queries = [
        `"${entity}" CAC RC number Nigeria registered company`,
        `"${noSuffix}" Nigeria company registration CAC`,
        `"${entity}" site:cac.gov.ng OR site:search.cac.gov.ng`,
        `"${entity}" RC Nigeria incorporated`,
      ];
      for (const q of queries) {
        ...
        for (const item of items) {
          ...
          if (rcMatch) {
            // Also try to extract incorporation date, address
            const dateMatch = text.match(/incorporat\w+\s+(?:on\s+)?(\d{1,2}[\/-]\d{1,2}[\/-]\d{2,4}|\d{4})/i);
            const addrMatch = text.match(/(?:address|located|headquarter\w*)[:\s]+([^.]{10,80})/i);
            records = [{
              company_name: entity,
              rc_number: "RC" + rcMatch[1],
              status: "Active",
              address: addrMatch ? addrMatch[1].trim() : (item.snippet||"—"),
              date_of_incorporation: dateMatch ? dateMatch[1] : "—",
              _source: "serp_intelligence",
              _url: item.link,
            }];
            out.confidence = "medium";
            break;
      ...
      // Even if no RC found, return intelligence from search results
      if (!records.length && SERPAPI_KEY) {
        const items = await sSearch(`"${entity}" Nigeria company`, 3);
        if (items.length) {
          records = [{ company_name:entity, rc_number:"—", status:"Unverified", address:items[0].snippet||"—", _source:"serp_info" }];
          out.confidence = "low";
        }
      }
    }
    ...
    const dirVariants = [
      trimmed,
      trimmed.split(" ").slice(-1)[0],           // last name only
      trimmed.split(" ").slice(0,2).join(" "),    // first + last
      trimmed.split(" ").reverse().join(" "),     // reversed name order
    ];
    const dirEndpoints = [
      "https://search.cac.gov.ng/home/searchDirector?name=",
      "https://pre.cac.gov.ng/home/searchDirector?name=",
      "https://api.cac.gov.ng/api/v1/director-search?name=",
    ];
    outer2: for (const ep of dirEndpoints) {
      for (const v of dirVariants) {
        const {s,b} = await get(ep + encodeURIComponent(v));
        if (s===200 && b) { records = parseCACRecords(b); if (records.length) { out.confidence="high"; break outer2; } }
      }
    }
    // SerpApi fallback for individuals
    if (!records.length && SERPAPI_KEY) {
      const items = await sSearch(`"${trimmed}" Nigeria director company CAC`, 5);
      const companies = [];
      for (const item of items) {
        const text = (item.title||"") + " " + (item.snippet||"");
        const rcMatch = text.match(/RC[\s:\-]?(\d{4,8})/i);
        const coMatch = text.match(/(?:director|chairman|ceo|coo|cfo)\s+(?:of\s+)?([A-Z][A-Za-z\s&.,]{4,50}(?:Limited|Ltd|Plc|Nigeria))/i);
        if (coMatch || rcMatch) {
          companies.push({
            company: coMatch ? coMatch[1].trim() : entity,
  ...
  out.found = true;
  out.raw_records = records.slice(0,10);

  const r = records[0];

  if (isCompany) {
    // Deep extraction of all available fields
    let directors = [];
    if (Array.isArray(r.directors))
      directors = r.directors.map(d => typeof d==="string" ? d : (d.name||d.director_name||d.DirectorName||d.fullName||"")).filter(Boolean);
    else if (Array.isArray(r.Shareholders))
      directors = r.Shareholders.map(s => s.name||s).filter(Boolean);

    let shareholders = [];
    if (Array.isArray(r.shareholders))
      shareholders = r.shareholders.map(s => ({ name:s.name||s, percentage:s.percentage||s.shares||0, type:s.type||"" }));

    let secretary = extractField(r,"secretary","CompanySecretary","company_secretary");
    let auditor   = extractField(r,"auditor","Auditor","externalAuditor");

    // All matching records as related/similar companies
    const relatedCompanies = records.slice(1,8).map(x => ({
      ...
      type:   extractField(x,"company_type","CompanyType","type"),
    ...
    out.data = {
      ...
      status:       extractField(r,"status","CompanyStatus","companyStatus"),
      type:         extractField(r,"company_type","CompanyType","type","companyType"),
      address:      extractField(r,"address","RegisteredAddress","registeredAddress","office_address"),
      incorporated: extractField(r,"date_of_incorporation","DateOfIncorporation","incorporationDate","dateOfIncorporation"),
      email:        extractField(r,"email","Email","emailAddress"),
      phone:        extractField(r,"phone","Phone","telephone","phoneNumber"),
      lga:          extractField(r,"lga","LGA","localGovernment"),
      state:        extractField(r,"state","State","registeredState"),
      share_capital:extractField(r,"share_capital","ShareCapital","authorisedShareCapital"),
      directors, shareholders, secretary, auditor,
      related_companies: relatedCompanies,
      confidence: out.confidence,
      _source: r._source || "cac_api",
    };
  } else {
    const companies = records.map(x => ({
      company: extractField(x,"company_name","CompanyName","name","company"),
      rc:      extractField(x,"rc_number","RcNumber","rc"),
      role:    extractField(x,"role","designation","Designation","position"),
      status:  extractField(x,"status","CompanyStatus"),
    })).filter(x => x.company && x.company !== "—");
    out.data = { name:entity, companies, confidence:out.confidence };
  }
  return out;
}
