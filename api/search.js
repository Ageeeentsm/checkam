...
async function scanCAC(entity, isCompany){
  ...
  // ── RC SEARCH: resolve name first via OpenCorporates, then full burst ──
  let resolvedName = "";
  if(isRC){
    try{
      const ocRes = await serpFull(`RC${rcNum} site:opencorporates.com/companies/ng OR "RC ${rcNum}" Nigeria CAC company`, 5);
      const ocHits = (ocRes.organic||[]);
      // Try to extract company name from OC snippet: "Company Name - RC12345 - Nigeria"
      for(const r of ocHits){
        const t = (r.title||"")+" "+(r.snippet||"");
        // OC title format: "Company Name - 12345 (Nigeria)"
        const ocName = (r.title||"").replace(/\s*[-–]\s*\d{4,8}\s*\(Nigeria\).*/i,"").replace(/\s*\|.*$/,"").trim();
        if(ocName && ocName.length>3 && !/opencorporates/i.test(ocName)){
          resolvedName = ocName;
          break;
        }
        // Try snippet patterns: "RC12345 is registered as CompanyName"
        const snM = t.match(/(?:registered as|company name[:\s]+|trading as)\s+([A-Z][A-Za-z\s&.,]+(?:Limited|Ltd|PLC|Nigeria|Group)[A-Za-z\s.]*)/i);
        if(snM){ resolvedName = snM[1].trim(); break; }
      }
      // Also try KG
      if(!resolvedName && ocRes.kg?.title && !/opencorporates/i.test(ocRes.kg.title)){
        resolvedName = ocRes.kg.title;
      }
    } catch(_){}
  }
  
  // Use resolved name OR RC number for all downstream queries
  ...
  const queries = isRC ? [
    // 1. RC number direct + OpenCorporates
    `"RC${rcNum}" site:opencorporates.com/companies/ng OR RC${rcNum} Nigeria CAC company registered`,
    // 2. Resolved name queries (if we found the name)
    resolvedName ? `"${resolvedName}" directors chairman CEO "managing director" board Nigeria` : `RC${rcNum} directors chairman CEO Nigeria company`,
    // 3. Contact info for resolved name
    resolvedName ? `"${resolvedName}" Nigeria address phone website "contact us"` : `RC${rcNum} Nigeria address headquarters`,
    // 4. News / appointments
    resolvedName ? `"${resolvedName}" Nigeria appointed directors officers annual report` : `RC${rcNum} Nigeria annual report`,
  ] : [
  ...
  // Fire main queries + foot traffic in parallel
  const [results, footTrafficResult] = await Promise.all([
    Promise.all(queries.map(q => serpFull(q, 8))),
    serpFull(`"${trimmed}" Nigeria "opening hours" OR "busy hours" OR "foot traffic" OR "visitors per day" OR "footfall" OR "crowded" OR "peak hours" OR "how busy"`, 5)
      .catch(()=>({organic:[]}))
  ]);
  
  // Extract foot traffic signals
  let footTraffic = null;
  const ftSnippets = (footTrafficResult.organic||[]);
  const ftText = ftSnippets.map(r=>`${r.title} ${r.snippet}`).join(" ");
  // Check if SerpApi returned popular_times in local results
  if(footTrafficResult.local_results){
    const lr = (footTrafficResult.local_results||[]).find(r=>(r.title||"").toLowerCase().includes(bare.toLowerCase().slice(0,6)));
    if(lr?.popular_times) footTraffic = { source: "google_maps", data: lr.popular_times };
  }
  // Fallback: extract text mentions of busy times
  if(!footTraffic && ftText.length > 50){
    const peakM = ftText.match(/(?:peak|busiest|most.*busy|crowded)[^.]{0,60}(?:morning|afternoon|evening|monday|tuesday|\d+am|\d+pm)[^.]{0,40}/gi);
  ...
  if(!website){
    // Domains we NEVER want as the "official website"
  const SKIP = [
    // Social/comms platforms — NOT official company sites
    "zoom.us","meet.google","teams.microsoft","webex","skype",
    "facebook","instagram","twitter","x.com","tiktok","youtube","linkedin",
    // Aggregators & directories
    "businesslist","companiesng","naijafirms","vconnect","hotfrog","yellowpages",
    "sabi","ngcareers","jobberman","nairalist","businessday.ng","nairametrics",
    "vanguard","punch","guardian","premiumtimes","dailytrust","thecable","channels",
    "thisdaylive","sundiata","informationng","naijaloaded","legit.ng","pulse.ng",
    // Regulatory/gov — real but not the company's site
    "efcc.gov","icpc.gov","cac.gov","sec.gov","cbn.gov","firs.gov","ngxgroup",
    // Generic platforms
    "google","wikipedia","amazon","microsoft","apple","wordpress","blogspot",
    "wix.com","weebly","squarespace","godaddy","namecheap",
    // Review/mapping
  ...
  allSnippets.filter(r=>(r.link||"").includes("linkedin")).forEach(r=>{
    ...
    // Prefer official Nigerian domains (.com.ng, .ng, .org) over generic ones
    const webCandidates = allSnippets
      .map(r=>r.displayLink||"")
      .filter(d=> d && !SKIP.some(sk=>d.toLowerCase().includes(sk)));
    // Score: .com.ng or .ng = 3pts, .org = 2pts, domain matches entity name = 2pts, else 1pt
    const bare2 = bare.toLowerCase().replace(/[^a-z]/g,"");
    const scored = webCandidates.map(d=>{
      let sc = 1;
      const dl = d.toLowerCase();
      if(dl.endsWith(".ng")||dl.endsWith(".com.ng")) sc+=3;
      if(dl.endsWith(".org")||dl.endsWith(".co")) sc+=2;
      if(dl.replace(/[^a-z]/g,"").includes(bare2.slice(0,6))) sc+=2;
      return {d, sc};
    }).sort((a,b)=>b.sc-a.sc);
    if(scored.length) website = "https://"+scored[0].d;
  }

  // ── OPENCORPORATES DIRECT LOOKUP (best RC source) ───────────────
  if(!rcNumber || confidence !== "high") {
    try {
      // OC has a free JSON API for Nigerian companies
      const ocQ = encodeURIComponent(trimmed);
      const {s:ocS, b:ocB} = await get(`https://api.opencorporates.com/v0.4/companies/search?q=${ocQ}&jurisdiction_code=ng&per_page=5`);
      if(ocS===200 && ocB){
        const ocJ = JSON.parse(ocB);
        const companies = ocJ?.results?.companies || [];
        // Find best match by name similarity
        const match = companies.find(c=>{
          const n = (c.company?.name||"").toLowerCase();
          const b2 = bare.toLowerCase();
          return n.includes(b2.slice(0,8)) || b2.includes(n.slice(0,8));
        }) || companies[0];
        if(match?.company){
          const oc = match.company;
          const ocRC = (oc.company_number||"").replace(/^0+/,"");
          if(ocRC && !rcNumber){ rcNumber = "RC"+ocRC; confidence = "high"; }
    ...
    } catch(_) { /* OC lookup failed, continue with what we have */ }
  }
  ...
  // Determine best display name: resolved > extracted from snippets > original query
  const bestName = resolvedName 
    || (isRC && allSnippets.find(r=>r.title && !/opencorporates|businesslist|companiesng/i.test(r.title))?.title?.split(/[-|]/)[0]?.trim())
    || trimmed;
...
// ── ENTITY PHOTO LOOKUP ─────────────────────────────────────────
// Wikipedia cross-check — returns null if unverified (no photo > wrong photo)
async function getEntityPhoto(name){
  try {
    const wikiSlug = name.trim().replace(/\s+/g,"_");
    const {s:ws,b:wb} = await get(`https://en.wikipedia.org/api/rest_v1/page/summary/${encodeURIComponent(wikiSlug)}`);
    if(ws===200 && wb){
      const w = JSON.parse(wb);
      const nameParts = name.toLowerCase().split(" ").filter(p=>p.length>2);
      const titleLower = (w.title||"").toLowerCase();
      const titleMatch = nameParts.filter(p=>titleLower.includes(p)).length >= Math.min(2,nameParts.length);
      if(titleMatch && w.thumbnail?.source){
        return {
          url: w.thumbnail.source.replace(/\/\d+px-/,"/220px-"),
          source:"wikipedia",
...
module.exports=async function handler(req,res){
  ...
  try{
    ...
    const[geo,network,photo]=await Promise.all([
