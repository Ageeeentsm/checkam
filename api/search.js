...
async function scanCAC(entity, isCompany){
  ...
  if(isRC){
    ...
    if(!resolvedName){
      try{
        // OC indexed pages have RC in URL and company name in title — most reliable
        const res = await serpFull(`RC${rcNum} site:opencorporates.com/companies/ng OR "RC${rcNum}" Nigeria CAC company`, 6);
        ...
        if(!resolvedName){
          for(const r of hits){
            ...
            // OC URL: extract from link if possible
            const ocUrlM = (r.link||"").match(/opencorporates\.com\/companies\/ng\/(\d+)/i);
            if(ocUrlM && String(ocUrlM[1]) === String(rcNum)) {
              // This OC page IS for our RC - title is the company name
              const clean = title
