...
async function scanCAC(entity, isCompany){
  ...
  if(isRC){
    try{
      // Try OpenCorporates API directly first (free, structured)
      const {s:ocS,b:ocB} = await get(`https://api.opencorporates.com/v0.4/companies/ng/${rcNum}`);
      if(ocS===200 && ocB){
        const oc = JSON.parse(ocB)?.results?.company;
        if(oc?.name){ resolvedName = oc.name; }
      }
    }catch(_){}

    // Fallback: SerpApi search for the RC number
    if(!resolvedName){
      try{
        const res = await serpFull(`"RC${rcNum}" Nigeria company CAC registered`, 6);
        const hits = res.organic||[];
        // KG title is most reliable
        if(res.kg?.title && !/opencorporates|businesslist|companiesng|naijafirms/i.test(res.kg.title)){
          resolvedName = res.kg.title;
        }
        if(!resolvedName){
          for(const r of hits){
            const title = (r.title||"").trim();
            // OC format: "COMPANY NAME - 123456 (Nigeria)" or "COMPANY NAME | OpenCorporates"
            const clean = title
              .replace(/\s*[-–|]\s*\d{4,8}\s*\(?Nigeria\)?.*$/i,"")
              .replace(/\s*[|].*$/,"")
              .trim();
            if(clean && clean.length > 3 && clean.length < 80 &&
               !/opencorporates|businesslist|naijafirms|companiesng|google/i.test(clean)){
              resolvedName = clean;
              break;
            }
          }
...
async function getEntityPhoto(name){
  // ── Try Wikipedia first (most verified) ──
  try {
    ...
    if(ws===200 && wb){
      ...
      if(titleMatch && w.thumbnail?.source){
        return {
          url: w.thumbnail.source.replace(/\/\d+px-/,"/240px-"),
          source:"wikipedia",
  ...
  // ── Try SerpApi image search for LinkedIn profile photo ──
  if(SERPAPI_KEY){
    try{
      const imgRes = await serpFull(`"${name}" site:linkedin.com/in Nigeria`, 3);
      // SerpApi images endpoint — check knowledge graph image
      if(imgRes.kg?.image){
        // Verify: KG title must contain name parts
        const nameParts = name.toLowerCase().split(" ").filter(p=>p.length>2);
        const kgTitle = (imgRes.kg.title||"").toLowerCase();
        if(nameParts.filter(p=>kgTitle.includes(p)).length >= Math.min(2,nameParts.length)){
          return { url: imgRes.kg.image, source:"knowledge_graph", caption: imgRes.kg.description||"", verified:true };
        }
      }
      // Check organic snippets for LinkedIn thumbnail
      const liHit = (imgRes.organic||[]).find(r=>(r.link||"").includes("linkedin.com/in/"));
      if(liHit?.thumbnail){
        return { url: liHit.thumbnail, source:"linkedin", caption: liHit.snippet||"", verified:true };
      }
    }catch(_){}
  }
  return null;
